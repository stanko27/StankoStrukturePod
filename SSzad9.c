...
/* veci ide lijevo, manji ili isti desno (prema slici zadatka) */
...
/* inorder ispis, vraca broj ispisanih elemenata */
...
/* replace: vraca sumu starog podstabla */
...
/* oslobadanje stabla, vraca broj obrisanih cvorova */
int FreeTree(treePointer root)
{
    if (!root)
        return 0;

    int count = 0;
    count += FreeTree(root->left);
    count += FreeTree(root->right);
    free(root);
    count++;
